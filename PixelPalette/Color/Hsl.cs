...
public readonly partial struct Hsl : IEquatable<Hsl>
{
    ...
    [GeneratedRegex("^hsl\\(\\s*(?<hue>0|1(?:\\.0)?|0?\\.\\d+)\\s*,\\s*(?<sat>0|1(?:\\.0)?|0\\.\\d+)\\s*,\\s*(?<lum>0|1(?:\\.0)?|0\\.\\d+)\\s*\\)$", RegexOptions.IgnoreCase,
                    "en-US")]
    private static partial Regex StringPatternRegex();

    [GeneratedRegex("^hsl\\(\\s*(?<hue>\\d+(?:\\.\\d+)?)\\s*,\\s*(?<sat>\\d+(?:\\.\\d+)?)%?\\s*,\\s*(?<lum>\\d+(?:\\.\\d+)?)%?\\s*\\)$", RegexOptions.IgnoreCase, "en-US")]
