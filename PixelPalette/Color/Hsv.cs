...
/// HSV/HSB (same thing)
/// </summary>
public readonly partial struct Hsv : IEquatable<Hsv>
{
    ...
    [GeneratedRegex("^hsv\\(\\s*(?<hue>0|1(?:\\.0)?|0?\\.\\d+)\\s*,\\s*(?<sat>0|1(?:\\.0)|0\\.\\d+)\\s*,\\s*(?<val>0|1(?:\\.0)|0\\.\\d+)\\s*\\)$", RegexOptions.IgnoreCase,
                    "en-US")]
    private static partial Regex StringPatternRegex();

    [GeneratedRegex("^hsv\\(\\s*(?<hue>\\d+(?:\\.\\d+)?)\\s*,\\s*(?<sat>\\d+(?:\\.\\d+)?)%?\\s*,\\s*(?<val>\\d+(?:\\.\\d+)?)%?\\s*\\)$", RegexOptions.IgnoreCase, "en-US")]
