...
public readonly partial struct Cmyk : IEquatable<Cmyk>
{
    ...
    [GeneratedRegex("^cmyk\\(\\s*(?<c>0|1(?:\\.0)?|0?\\.\\d+)\\s*,\\s*(?<m>0|1(?:\\.0)?|0?\\.\\d+)\\s*,\\s*(?<y>0|1(?:\\.0)?|0?\\.\\d+)\\s*,\\s*(?<k>0|1(?:\\.0)?|0?\\.\\d+)\\s*\\)$", RegexOptions.IgnoreCase, "en-US")]
    private static partial Regex StringPatternRegex();
    [GeneratedRegex("^cmyk\\(\\s*(?<c>\\d+(?:\\.\\d+)?)%?\\s*,\\s*(?<m>\\d+(?:\\.\\d+)?)%?\\s*,\\s*(?<y>\\d+(?:\\.\\d+)?)%?\\s*,\\s*(?<k>\\d+(?:\\.\\d+)?)%?\\s*\\)$", RegexOptions.IgnoreCase, "en-US")]
