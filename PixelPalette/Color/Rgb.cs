...
public readonly partial struct Rgb : IEquatable<Rgb>
{
    ...
    {
        return ValidStringRegex().IsMatch(theString);
    }

    public static bool IsValidScaledString(string theString)
    {
        return ValidScaledStringRegex().IsMatch(theString);
    }

    public static bool IsValidComponent(string theString)
    {
        return ValidComponentRegex().IsMatch(theString);
    }
    ...
    {
        var match = StringPatternRegex().Match(theString);
        if (match.Success)
        {
            return new Rgb(
    ...
    {
        var match = ScaledStringPatternRegex().Match(theString);
    ...
    [GeneratedRegex("^rgb\\(\\s*(?<red>0|1|0?\\.\\d+)\\s*,\\s*(?<green>0|1|0\\.\\d+)\\s*,\\s*(?<blue>0|1|0\\.\\d+)\\s*\\)$")]
    private static partial Regex ValidStringRegex();

    [GeneratedRegex("^rgb\\(\\s*(?<red>\\d{1,3})\\s*,\\s*(?<green>\\d{1,3})\\s*,\\s*(?<blue>\\d{1,3})\\s*\\)$")]
    private static partial Regex ValidScaledStringRegex();

    [GeneratedRegex("^0$|^1(\\.0)?$|^0?\\.\\d+$")]
    private static partial Regex ValidComponentRegex();

    [GeneratedRegex("^rgb\\(\\s*(?<red>0|1|0?\\.\\d+)\\s*,\\s*(?<green>0|1|0\\.\\d+)\\s*,\\s*(?<blue>0|1|0\\.\\d+)\\s*\\)$", RegexOptions.IgnoreCase, "en-US")]
    private static partial Regex StringPatternRegex();

    [GeneratedRegex("^rgb\\(\\s*(?<red>\\d{1,3})\\s*,\\s*(?<green>\\d{1,3})\\s*,\\s*(?<blue>\\d{1,3})\\s*\\)$", RegexOptions.IgnoreCase, "en-US")]
    private static partial Regex ScaledStringPatternRegex();
}
